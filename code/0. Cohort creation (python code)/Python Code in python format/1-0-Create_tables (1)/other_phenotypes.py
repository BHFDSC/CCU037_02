...
medications_table = ".primary_care_meds_"

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM 
# MAGIC .hes_apc_all_years_archive LIMIT 10

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM 
# MAGIC .gdppr__archive LIMIT 10
...
# MAGIC FROM .gdppr__archive WHERE ProductionDate = ProductionDate AND NHS_NUMBER_DEID IS NOT NULL
...
# MAGIC SELECT Person_ID_DEID AS NHS_NUMBER_DEID, ProcessingPeriodDate AS date, PrescribedBNFCode AS code FROM .primary_care_meds_ WHERE Person_ID_DEID IS NOT NULL
...
# MAGIC LEFT JOIN 
# MAGIC   .ccu037_cohort_all_inclusion_criteria b
# MAGIC ON
...
#     LEFT JOIN .ccu037_cohort_all_inclusion_criteria as b
...
# MAGIC %run /Workspaces//CCU037_01/Create_skinny_table_using_013_code/CCU037_00_helper_functions
...
# MAGIC %sql
# MAGIC SELECT DISTINCT(predictor), COUNT(predictor) AS count FROM .ccu037_other_qrisk_phenotypes GROUP BY predictor
