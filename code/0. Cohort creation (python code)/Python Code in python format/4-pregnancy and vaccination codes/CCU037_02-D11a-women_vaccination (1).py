...
#%run "/Workspaces//CCU036_02/CCU036_02-D01-parameters"
...
# MAGIC select * from  .ccu037_02_codelist_vaccine_products 

# COMMAND ----------

# DBTITLE 1,Vaccination source table in TRE
# MAGIC %sql 
# MAGIC select COUNT(distinct PERSON_ID_DEID) from ._vaccine_status_ --Similar number than the total amount of individuals who had been vacinated in England

# COMMAND ----------

#Comparison to ccu036 frozen
#%sql
#select count(DISTINCT PERSON_ID_DEID) from .ccu036_01_in_vaccine_status__archive_pre20220522 -- this n is much lower
#%sql
#select count(DISTINCT PERSON_ID_DEID) from .ccu036_01_in_vaccine_status__archive -- this n is much lower

# COMMAND ----------

# DBTITLE 1,Using "ccu036_01_in_vaccine_status__archive_pre20220522" as archive frozen table as it works for us
# MAGIC %sql
...
# MAGIC FROM ._vaccine_status_ AS vaccination_raw
# MAGIC -- FROM .ccu036_01_in_vaccine_status__archive_pre20220522 AS vaccination_raw
# MAGIC -- LEFT JOIN (SELECT code, name FROM .ccu002_06_codelists WHERE left(name,16)="COVID19_vaccine_") AS product ON vaccination_raw.VACCINE_PRODUCT_CODE = product.code
# MAGIC -- LEFT JOIN (SELECT * FROM .ccu002_06_codelists WHERE left(name,16)="COVID19_vaccine_") AS procedure ON vaccination_raw.VACCINATION_PROCEDURE_CODE = procedure.code
# MAGIC -- LEFT JOIN (SELECT code, name FROM .ccu002_06_codelists WHERE left(name,16)="COVID19_vaccine_") AS situation ON vaccination_raw.VACCINATION_SITUATION_CODE = situation.code
# MAGIC LEFT JOIN (SELECT code, name FROM .ccu037_02_codelist_vaccine_products WHERE left(name,16)="COVID19_vaccine_") AS product ON vaccination_raw.VACCINE_PRODUCT_CODE = product.code
# MAGIC LEFT JOIN (SELECT * FROM .ccu037_02_codelist_vaccine_products WHERE left(name,16)="COVID19_vaccine_") AS procedure ON vaccination_raw.VACCINATION_PROCEDURE_CODE = procedure.code
# MAGIC LEFT JOIN (SELECT code, name FROM .ccu037_02_codelist_vaccine_products WHERE left(name,16)="COVID19_vaccine_") AS situation ON vaccination_raw.VACCINATION_SITUATION_CODE = situation.code;
# MAGIC 
...
# MAGIC %sql
# MAGIC -- Restrict ccu037_02_vaccination to individuals present in .ccu037_02_cohort
# MAGIC CREATE OR REPLACE GLOBAL TEMP VIEW ccu037_02_vaccination_gdppr AS 
# MAGIC SELECT * -- AS NHS_NUMBER_DEID,
# MAGIC FROM (SELECT NHS_NUMBER_DEID as ID, age, sex FROM .ccu037_02_cohort) as cohort
...
# MAGIC %sql
# MAGIC -- Restrict ccu037_02_vaccination to WOMEN present in .ccu037_02_cohort
...
# MAGIC %sql
# MAGIC describe extended .ccu037_02_vaccination_gdppr_women
