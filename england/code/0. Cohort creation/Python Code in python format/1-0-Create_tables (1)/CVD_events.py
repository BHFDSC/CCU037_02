...
# MAGIC %sql
# MAGIC SELECT * FROM .ccu013_caliber_category_mapping
...
CREATE OR REPLACE GLOBAL TEMP VIEW phenotype_categories AS
   ...
   FROM .ccu037_caliber AS a
   LEFT JOIN .ccu013_caliber_category_mapping as b
...
# MAGIC %sql
# MAGIC select count(distinct person_id_deid) from .ccu037_caliber 
...
CREATE OR REPLACE GLOBAL TEMP VIEW ccu037_post_covid_cvd_events_list AS
    ...
    LEFT JOIN .ccu037_inclusion_criteria_updated as b
...
CREATE OR REPLACE GLOBAL TEMP VIEW ccu037_90_day_post_covid_cvd_events AS
    SELECT person_id_deid, "90_day_CVD_event" as predictor FROM
    (SELECT a.person_id_deid, a.phenotype, a.date, a.code, a.terminology, a.category, b.date_first_covid_event
    FROM global_temp.CVD_phenotypes AS a
    LEFT JOIN .ccu037_inclusion_criteria_updated as b
    ON a.person_id_deid = b.NHS_NUMBER_DEID)
...
CREATE OR REPLACE GLOBAL TEMP VIEW ccu037_180_day_post_covid_cvd_events AS
    SELECT person_id_deid, "180_day_CVD_event" as predictor FROM
    (SELECT a.person_id_deid, a.phenotype, a.date, a.code, a.terminology, a.category, b.date_first_covid_event
    FROM global_temp.CVD_phenotypes AS a
    LEFT JOIN .ccu037_inclusion_criteria_updated as b
    ON a.person_id_deid = b.NHS_NUMBER_DEID)
...
CREATE OR REPLACE GLOBAL TEMP VIEW ccu037_1_year_post_covid_cvd_events AS
    SELECT person_id_deid, "1_year_CVD_event" as predictor FROM
    (SELECT a.person_id_deid, a.phenotype, a.date, a.code, a.terminology, a.category, b.date_first_covid_event
    FROM global_temp.CVD_phenotypes AS a
    LEFT JOIN .ccu037_inclusion_criteria_updated as b
    ON a.person_id_deid = b.NHS_NUMBER_DEID)
...
CVD = spark.sql(f"""
  SELECT person_id_deid, predictor, 1 as value FROM GLOBAL_TEMP.ccu037_all_cvd_events """)
...
CVD = spark.sql(f"""
  SELECT person_id_deid, predictor, 1 as value FROM .ccu037_all_cvd_events """)
...
FROM .ccu037_inclusion_criteria_updated a
...
# MAGIC LEFT JOIN .ccu037_inclusion_criteria_updated as b
...
# MAGIC %sql
# MAGIC SELECT * FROM .ccu037_cvd_events_and_dates
...
# cvd events 30 days after infection
spark.sql(f"""
...
FROM .ccu037_cohort_base_nhs_numbers as a
LEFT JOIN
...
SELECT * FROM global_temp.ccu037_2_years_post_covid_cvd_events
UNION ALL
SELECT * FROM GLOBAL_TEMP.VTE_AND_CVD
""")
...
CVD.createOrReplaceGlobalTempView("ccu037_cvd_events_updated")

# COMMAND ----------

# MAGIC %sql
# MAGIC select count(*) from global_temp.ccu037_cvd_events_updated

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from global_temp.ccu037_cvd_events_updated LIMIT 10

# COMMAND ----------

drop_table("ccu037_cvd_events_updated")
create_table("ccu037_cvd_events_updated")
...
FROM .ccu037_cvd_events_and_dates a
...
# MAGIC SELECT DISTINCT predictor FROM .ccu037_all_cvd_events

# COMMAND ----------

# MAGIC %sql
# MAGIC select count(*) from global_temp.ccu037_cvd_outcomes_with_dates_3