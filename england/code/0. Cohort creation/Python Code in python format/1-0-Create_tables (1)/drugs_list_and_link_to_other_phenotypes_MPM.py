...
# MAGIC They obtained the llists from the table 'ccu002_03_primary_care_meds_' with is a freezed table from 'primary_care_meds_'
# MAGIC 
...
# MAGIC SELECT * FROM .primary_care_meds_ WHERE (Person_ID_DEID IS NOT NULL AND left(PrescribedBNFCode, 9) IN (SELECT BNFcode FROM global_temp.antidiabeticBNF) )
...
# MAGIC SELECT * FROM .primary_care_meds_ WHERE (Person_ID_DEID IS NOT NULL AND PrescribeddmdCode IN (SELECT code FROM global_temp.hypertension_drugs) )

# COMMAND ----------

# MAGIC %sql  -- SEE if we found BNF codes with dmd
# MAGIC SELECT PrescribeddmdCode, PrescribedBNFCode FROM .primary_care_meds_ WHERE (PrescribeddmdCode IN (SELECT code FROM global_temp.hypertension_drugs) )
...
# MAGIC %sql -- SEE there are records
# MAGIC SELECT * FROM .primary_care_meds_ WHERE (Person_ID_DEID IS NOT NULL AND left(PrescribedBNFCode, 9) IN (SELECT BNFcode FROM global_temp.antihypertensiveBNF))
...
# MAGIC SELECT * FROM .primary_care_meds_ WHERE 
...
# MAGIC SELECT DISTINCT * FROM .ccu002_03_primary_care_meds_
...
# MAGIC %sql
# MAGIC SELECT DISTINCT PrescribedBNFCode AS code, PrescribedBNFName AS term, 'BNF' AS system, 'statin_drugs' AS codelist FROM .primary_care_meds_ WHERE (left(PrescribedBNFCode, 7) =  "0212000"   AND NOT (left(PrescribedBNFCode, 9) = '0212000U0')) -- Nicotinic acid (is a nutrient, not an statin)

# COMMAND ----------

# MAGIC %sql -- See there are records
# MAGIC SELECT * FROM .primary_care_meds_ WHERE (Person_ID_DEID IS NOT NULL AND left(PrescribedBNFCode, 7) =  "0212000"   AND NOT (left(PrescribedBNFCode, 9) = '0212000U0')) -- Nicotinic acid (is a nutrient, not an statin)
