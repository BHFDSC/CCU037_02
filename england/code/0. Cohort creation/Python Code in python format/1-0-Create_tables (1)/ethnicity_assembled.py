...
# MAGIC  
# MAGIC **Data input**   
# MAGIC * gdppr_data =  `.gdppr__archive` 
# MAGIC * hes_data = `.hes_apc_all_years`  
# MAGIC     
# MAGIC **Data output**  
# MAGIC * `.ccu037_ethnicity_assembled` 1 row per patient
# MAGIC 
...
gdppr_prod_date = "2022-06-29 00:00:00.000000"
hes_prod_date ="2022-06-29 00:00:00.000000"
...
spark.sql(f"""
 ...
 SELECT DISTINCT PERSON_ID_DEID as NHS_NUMBER_DEID, 
      ...
      3 as eth_rank,
      recent_rank
  FROM (select *, row_number() over (partition by PERSON_ID_DEID order by (CASE WHEN ETHNOS IS NULL or TRIM(ETHNOS) IN ("","9", "99", "X" , "Z") THEN 1 ELSE 0 END), EPISTART desc) as recent_rank from .hes_apc_all_years_archive 
...
spark.sql(f"""
 CREATE OR REPLACE GLOBAL TEMP VIEW ccu037_ethnicity_assembled_all_codes AS
...
RECORD_DATE, 
dataset 
FROM global_temp.ccu037_ethnicity_assembled_all_codes """)
