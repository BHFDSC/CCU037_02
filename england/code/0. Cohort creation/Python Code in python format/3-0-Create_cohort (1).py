...
# MAGIC select distinct person_id_deid, min(date) as min_hosp_date, 1 as covid_hosp_check from .ccu013_covid_trajectory
# MAGIC where 
...
# MAGIC select *
# MAGIC from .ccu037_inclusion_criteria_updated a
...
# MAGIC %sql
# MAGIC select * from .ccu037_cvd_events_updated
...
# MAGIC %sql
# MAGIC select count(*) from .ccu037_inclusion_criteria_updated
...
SELECT * FROM .ccu037_all_cvd_events WHERE predictor LIKE "%1_year_pre_covid_CVD_event%" OR predictor LIKE "%pre_covid_CVD_event%"
""")
...
CREATE OR REPLACE GLOBAL TEMP VIEW predictors AS
   SELECT a.NHS_NUMBER_DEID, b.predictor
     ...
     LEFT JOIN
      (SELECT * FROM .ccu037_variables_phenotypes_list_final3
      UNION ALL
      SELECT * FROM .ccu037_medications_list_final
      UNION ALL
      SELECT * FROM global_temp.cvd_predictors
      UNION ALL
      SELECT * FROM .ccu037_other_qrisk_phenotypes) AS b
...
# MAGIC LEFT JOIN .curr901a_lsoa_region_lookup AS geog ON lsoa.lsoa_code = geog.lsoa_code
